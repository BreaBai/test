...
public class RandCodeImageUtils {
    ...
    private static final int math_width = 145;
	/**
	 * 定义图形大小
	 */
	...
	public static String getKey(){
		...
		return SESSION_KEY_OF_RAND_CODE ;
	}
...
//			graphics.setFont(new Font("Arial Black", Font.ITALIC, 18));
	...
	public static void generateImageMath(HttpServletResponse response, HttpServletRequest request) throws IOException {
		...
		// 在内存中创建图象
		final BufferedImage image = new BufferedImage(math_width, height, BufferedImage.TYPE_INT_RGB);
		// 获取图形上下文
		final Graphics2D graphics = (Graphics2D) image.getGraphics();

		// 设定背景颜色
		graphics.setColor(Color.WHITE); // ---1
		graphics.fillRect(0, 0, math_width, height);
		// 设定边框颜色
//		graphics.setColor(getRandColor(100, 200)); // ---2
		graphics.drawRect(0, 0, math_width - 1, height - 1);
		...
		for (int i = 0; i < count; i++) {
			...
			final int x = random.nextInt(math_width - lineWidth - 1) + 1; // 保证画在边框之内
			final int y = random.nextInt(height - lineWidth - 1) + 1;
			final int xl = random.nextInt(lineWidth);
			final int yl = random.nextInt(lineWidth);
			graphics.drawLine(x, y, x + xl, y + yl);
		...
		// ToDo 需要删除
		final List<String> mathCodeList = MathRandCodeImage.createText();
		final String resultCode = mathCodeList.get(0);
		final String validCodeResult = mathCodeList.get(1);
		for (int i = 0; i < resultCode.length(); i++) {
		...
		System.out.println("验证码："+resultCode);
		request.getSession().setAttribute(SESSION_KEY_OF_RAND_CODE, validCodeResult.toUpperCase());
		// 图象生效
		graphics.dispose();
		// 输出图象到页面
		ImageIO.write(image, "JPEG", response.getOutputStream());
 ...
 * 生成加减法的验证码
 */
class MathRandCodeImage {


	/**
	 * 数组第一位存的是加减过程，第二位是最后的结果
	 * @return
     */
	public static List<String> createText() {
		...
		intFirst = (int) (Math.random() * 100);
		intSec = (int) (Math.random() * 100);
